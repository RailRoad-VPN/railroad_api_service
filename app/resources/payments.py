...
class PaymentsAPI(ResourceAPI):
    ...
    def post(self) -> Response:
        ...
        self.logger.debug(f"Create APN file name: {tt}")
        with open(self._config['APN_PATH'] % tt, 'w') as file:
            file.write(apn)
            file.close()

        self.logger.debug(f"decode APN base64 string to utf-8")
        apn_str = base64.b64decode(apn).decode("utf-8")
        json_data_dict = {}
        self.logger.debug(f"convert APN to dict")
        ...
        self.logger.debug(f"got: {order_custom_fields}")

        self.logger.debug(f"processing order_custom_fields. try to find x-ordercode and x-useruuid fields")
        order_code = None
        user_uuid = None
        for order_custom_field in order_custom_fields.split(','):
            self.logger.debug(f"processing custom field: {order_custom_field}")
        ...
        self.logger.debug(f"order_code: {order_code}")
        self.logger.debug(f"user_uuid: {user_uuid}")

        if order_code is None:
            self.logger.debug(f"failed to process APN {tt} because x-ordercode not in APN content")
            ...
            self.logger.debug(f"failed to process APN {tt} because x-useruuid not in APN content")
            resp = make_error_request_response(http_code=HTTPStatus.BAD_REQUEST,
                                               err=RailRoadAPIError.PAYMENT_APN_DOES_NOT_CONTAIN_USER_UUID_CUSTOM_FIELD)
            return resp

        try:
            self.logger.debug(f"get order by code")
        ...
        self.logger.debug(f"got order: {order}")
        order_uuid = order.get('uuid')

        self.logger.debug(f"get order status id from apn")
        apn_order_status_id = int(json_data_dict.get('ORDER_STATUS_ID'))
        self.logger.debug(f"got apn_order_status_id: {apn_order_status_id}")
        if apn_order_status_id == PPGPaymentStatus.PROCESSED.sid:
            self.logger.debug(f"it is {PPGPaymentStatus.PROCESSED.text} status: {PPGPaymentStatus.PROCESSED.sid}")
            ...
            self.logger.debug(f"it is {PPGPaymentStatus.WAITING.text} status: {PPGPaymentStatus.WAITING.sid}")
        ...
        self.logger.debug(f"create ppg payment")
        ...
        self.logger.debug(f"created payment uuid: {payment_uuid}")

        self.logger.debug(f"prepare updated order")
        order['status_id'] = order_status_id
        order['modify_reason'] = 'update status'
        self.logger.debug(f"updated order: {order}")

        self.logger.debug(f"update order")
        self._order_api_service.update_order(order_json=order)

        self.logger.debug(f"get all user subscriptions")
        api_response = self._user_sub_api_service.get_user_subs_by_user_uuid(user_uuid=user_uuid)
        user_subs = api_response.data
        self.logger.debug(f"got user subs: {user_subs}")

        self.logger.debug(f"find user subscription for which payment come by order_uuid")
        payment_user_sub = None
        for user_sub in user_subs:
            self.logger.debug(f"process user sub: {user_sub}")
            if user_sub['order_uuid'] == order_uuid:
                self.logger.debug(f"we found!")
        ...
        self.logger.debug(f"prepare updated user subscription")
        payment_user_sub['status_id'] = user_sub_status_id
        payment_user_sub['modify_reason'] = 'update status'
        self.logger.debug(f"updated user subscription: {payment_user_sub}")
        ...
        self.logger.debug(f"got user configurations response: {api_response.serialize()}")
